...
class CourseController extends Controller
{
    ...
    /**
     * Display a listing of available courses with caching.
     */
    public function index(Request $request)
    {
        try {
            // Generate cache key based on request parameters
            $cacheKey = 'courses_list_' . md5($request->fullUrl());
    ...
    {
        try {
            // Generate cache key based on course ID and user ID
            $userId = Auth::check() ? Auth::id() : 'guest';
            $cacheKey = "course_{$id}_user_{$userId}";

            return $this->cacheService->remember($cacheKey, function () use ($id, $request, $userId) {
                $course = Course::with([
    ...
    /**
     * Enroll the authenticated user in a course (no caching needed).
     */
    public function enroll($id, Request $request)
    {
        try {
            ...
            // Clear course cache after enrollment
            $this->cacheService->forget("course_{$id}_user_{$user->user_id}");
