...
{
    /**
     * Retrieve all required data for the home page.
     */
    public function index()
    {
        try {
            // 1. Fetch Featured Courses
            $featuredCourses = Course::with(['category', 'videos'])
            ...
            $popularCategories = Category::withCount('courses')
                ...
                ->map(function ($category) {
                    return [
                        ...
                        'icon' => $category->getImage(), // assuming image is used as icon
                        'courses_count' => $category->courses_count,
                    ];
                });

            // 3. Fetch Top Instructors
            $topInstructors = User::whereHas('instructorProfile')
            ...
            // 4. Fetch Limited Time Offers
            $deals = Course::whereHas('pricing', function ($query) {
