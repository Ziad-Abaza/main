...
@section('title', 'Assign Courses to Level - Dashboard')
        ...
        <div class="col-md-8 col-12">
            <div class="card shadow-lg border-radius-lg">
                <div class="card-header bg-white pb-0">
                    <h5 class="mb-0">Assign Courses to Level</h5>
                </div>
                <div class="card-body p-4">
                    <form action="{{ route('console.level-courses.store') }}" method="POST">
                        @csrf

                        <x-inputs.select name="level_id" label="Select Level"
                            :options="$levels->pluck('name', 'level_id')" required />

                        <x-inputs.select name="course_ids[]" label="Select Courses (Multiple Selection)"
                            :options="$courses->pluck('title', 'course_id')" multiple required />
