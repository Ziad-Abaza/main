<template>
  <div
    :class="[
      'min-h-screen',
      isDark ? 'bg-slate-900 text-white' : 'bg-blue-50 text-slate-900',
    ]"
  >
    <!-- Back Button -->
      ...
      <router-link
        ...
        :class="[
          ...
          isDark
            ...
            : 'bg-white border-blue-100 hover:border-blue-300 shadow',
        ]"
      >
    ...
    <div v-if="courseDetailsStore.loading" class="flex justify-center py-20">
      <div
        ...
        "
      ></div>
      <span :class="isDark ? 'ml-3 text-slate-400' : 'ml-3 text-slate-500'"
        >Loading course details...</span
      >
    </div>

    <!-- Error State -->
    <div v-else-if="courseDetailsStore.error" class="text-center py-20">
      <h3 :class="isDark ? 'text-red-400 text-xl' : 'text-red-600 text-xl'">
        {{ courseDetailsStore.error }}
      </h3>
    </div>
        ...
        <div class="lg:col-span-2 space-y-8">
          ...
          <div
            :class="[
              ...
              isDark ? 'glass-card-premium' : 'bg-white border border-blue-100',
            ]"
          >
            <img
            ...
            <div class="p-6">
              ...
              <p :class="isDark ? 'text-slate-300 mb-6' : 'text-slate-600 mb-6'">
                {{ course.description }}
              </p>
              ...
              <div class="flex items-center gap-4 mt-6">
                ...
                <div>
                  ...
                  <p
                    :class="isDark ? 'text-sm text-slate-400' : 'text-sm text-slate-500'"
                  >
                    {{ instructor.email }}
                  </p>
                </div>
          ...
          >
            ...
            <div>
              ...
              <div
                :class="isDark ? 'text-slate-300 mt-2' : 'text-slate-600 mt-2'"
                v-html="details.objectives"
              ></div>
            </div>
          ...
          >
            ...
            <div class="max-h-80 overflow-y-auto pr-2 custom-scrollbar space-y-2">
              <div
                ...
                :key="video.id"
                :class="[
                  'group flex items-center gap-4 p-3 rounded-lg cursor-pointer transition',
              ...
              >
                ...
                >
                  <font-awesome-icon
                    :icon="['fas', 'play']"
                    :class="isDark ? 'text-2xl text-blue-400' : 'text-2xl text-blue-600'"
                  />
                </div>
                <div class="flex-1">
                  <h3
                    :class="
                      isDark
                        ? 'font-semibold group-hover:text-blue-400 transition'
                        : 'font-semibold group-hover:text-blue-700 transition'
                    "
                  >
                    {{ video.title }}
                  </h3>
        ...
        <div class="space-y-6">
          ...
          >
            ...
            <div class="space-y-4">
              <div class="flex justify-between">
                ...
                <span
                  :class="
                    isDark ? 'text-slate-300 line-through' : 'text-slate-400 line-through'
                  "
                  >${{ pricing.original_price }}</span
                >
              </div>
              ...
              <div v-if="pricing.discounted_price" class="flex justify-between">
                ...
                <span
                  :class="
                    isDark ? 'text-yellow-300 font-bold' : 'text-yellow-700 font-bold'
                  "
                  >${{ pricing.discounted_price }}</span
                >
              </div>
              ...
              <div v-if="pricing.discount_percentage" class="mt-2">
                <span
                  :class="
                    isDark
                      ? 'inline-block bg-yellow-500/20 text-yellow-400 text-sm px-3 py-1 rounded-full'
                      : 'inline-block bg-yellow-100 text-yellow-700 text-sm px-3 py-1 rounded-full'
                  "
                >
              ...
              <div
                class="flex justify-between text-sm mt-4"
          ...
          <div v-if="enrolled" class="mb-6">
            <div
              :class="
                isDark
                  ? 'bg-green-900/80 border border-green-400 text-green-200'
                  : 'bg-green-50 border border-green-200 text-green-800'
              "
              class="rounded-xl px-6 py-5 flex flex-col md:flex-col md:items-center md:justify-between gap-4 shadow-lg animate__animated animate__fadeInDown"
            >
              <div class="flex items-center gap-3">
                <font-awesome-icon
                  icon="trophy"
                  :class="
                    isDark ? 'text-yellow-400 text-2xl' : 'text-yellow-600 text-2xl'
                  "
                />
                <span :class="isDark ? 'font-bold text-lg' : 'font-bold text-lg'"
                  >🎉 You're Enrolled!</span
                >
              </div>
              <div :class="isDark ? 'text-green-100' : 'text-green-700'">
                You have access to all course content. Start learning now!
              </div>
              <router-link
                :to="`/videos/${courseId}`"
                :class="
                  isDark
                    ? 'btn btn-primary bg-blue-600 hover:bg-blue-700 text-white font-semibold px-5 py-2 rounded-lg shadow transition'
                    : 'btn btn-primary bg-blue-500 hover:bg-blue-600 text-white font-semibold px-5 py-2 rounded-lg shadow transition'
                "
              >
                View All Videos
              </router-link>
          ...
          >
            ...
            <div class="space-y-4">
              ...
              <div
                class="pt-4 border-t"
                :class="isDark ? 'border-white/10' : 'border-blue-100'"
              >
                <button
                  @click="enroll"
                  :disabled="is_processing_enrollment || isEnrolling"
                  :class="[
                    'cursor-pointer w-full py-3 rounded-lg font-semibold transition-all duration-300 flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed',
                    isDark
                      ? 'bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white'
                      : 'bg-gradient-to-r from-blue-500 to-indigo-400 hover:from-blue-400 hover:to-indigo-500 text-white',
                  ]"
                >
          ...
          >
            ...
            <div v-if="coupons.length > 0">
              <p :class="isDark ? 'text-slate-300 mb-4' : 'text-slate-600 mb-4'">
                  ...
                  <input
                    ...
                    :class="[
                      ...
                      isDark
                        ...
                        : 'bg-blue-50 border border-blue-200 text-slate-900 placeholder:text-blue-400 focus:border-blue-500',
                    ]"
                    required
                  />
                  <button
                    type="submit"
                    :class="[
                      'cursor-pointer px-4 py-2 font-semibold rounded-lg transition-all duration-300',
                      isDark
                        ? 'bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white'
                        : 'bg-gradient-to-r from-purple-400 to-indigo-300 hover:from-purple-300 hover:to-indigo-400 text-white',
                    ]"
                  >
                    Apply Coupon
                  </button>
...
const is_processing_enrollment = computed(
  () => courseDetailsStore.course.is_processing_enrollment || false
);
